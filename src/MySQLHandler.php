...
class MySQLHandler extends AbstractProcessingHandler {
	 ...
	 */
	private $defaultfields = array('id', 'channel', 'level', 'message', 'app', 'server', 'created');
	...
	public function __construct(
		PDO $pdo = null, $table, $level = Logger::DEBUG, $app, $additionalFields = array(), $bubble = true
	) {
		if (!is_null($pdo)) {
			$this->pdo = $pdo;
		}
	...
	protected function write(array $record) {
		...
		*/
		...
		$contentArray = array_merge(array(
			...
			'created' => $record['datetime']->format('Y-m-d H:i:s'),
		), $record['context']);
		...
		$contentArray = $contentArray + array_combine(
			$this->additionalFields, array_fill(0, count($this->additionalFields), null)
		);
